...
export default function StandardProfileInterview({ profile, profileStatus, entryView, onDraftChange, onSave, onExplore }: Props) {
  ...
  return <main className={styles.page}><header className={styles.header}><div className={styles.brand}><span className={styles.brandMark}>知</span><span>知途留学</span></div><span className={styles.status}>{isEditing ? "更新背景信息" : "标准背景信息 · 不调用 AI"}</span></header><div className={styles.workspace}>
    ...
    <section className={styles.chat} aria-label="标准背景信息访谈"><div className={styles.chatHeader}><div className={styles.advisorAvatar}>顾</div><div><strong>申请顾问小知</strong><span><i /> 正在记录通用用户事实</span></div><span className={styles.stepCount}>{isEditing ? "单项修改" : `${step + 1} / ${STEPS.length}`}</span></div><div className={styles.messages} aria-live="polite"><div className={styles.dayLabel}>今天</div>
...
function summary(profile: StandardUserProfile, module: StandardProfileModule): string {
  ...
  return `CV：${profile.materials.cv_status ?? "未填写"} · 推荐人：${profile.materials.confirmed_recommenders ?? "未填写"}`;
}
