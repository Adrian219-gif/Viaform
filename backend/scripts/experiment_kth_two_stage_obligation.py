...
def main():
    ...
    try:
        with httpx.Client(timeout=600, trust_env=False, follow_redirects=False) as client:
            ...
            instructions += (
                '\nOBLIGATION / IMPORTANCE RUBRIC: Use importance=required only when the evidence '
                'clearly expresses an obligation: must, required, mandatory, shall, need to, has to, '
                'prerequisite, condition for admission, must submit/provide, required to complete. '
                'Permission, eligibility and optional paths are not obligations: may, can, optional, '
                'eligible to, possible to, applicants may apply for, students can choose, may request, '
                'can be used, opportunity to. Never upgrade these to required merely because they occur '
                'on an admissions or entry-requirements page. Judge the proposition in context, not an '
                'isolated keyword: an optional way to satisfy an explicitly mandatory requirement does '
                'not make the underlying requirement optional, but it does not make that particular '
                'way mandatory either. If obligation is unclear, choose the most faithful conservative '
                'existing value. Existing values are required, recommended, preferred, unknown; there '
                'is no optional importance enum. Use unknown for mere permission unless recommendation '
                'or preference is actually evidenced. Do not invent fields or unsupported recommendations. '
                'Only Call 1 evidence may justify obligation. Never infer that a document is usually '
                'mandatory from common knowledge or model memory. Important-looking information does '
                'not imply required. Never rewrite may apply as must apply, or can use as must use.'
            )
    ...
    (ROOT / 'docs/evals/kth_two_stage_obligation.json').write_text(text, encoding='utf-8')
